...
/**
    1. Keep the node val & index // structure [idx, val]
    2. For node i, it's index is 2*parentIdx (left) & 2*parentIdx + 1(right)
    3. mapping the idx/val structure to array with null placeholde if idx did not exist
...
function serialize(root: TreeNode | null): string {
	const bfs = (root: TreeNode | null): string[] => {
		if (root === null) return [];
		const ret: string[] = [];
		const queue: [number, TreeNode | null][] = [];
		queue.push([0, root]);
		...
		while (queue.length !== 0) {
			...
			while (counts-- !== 0) {
				const firstMeta = queue.shift();
				const [idx, node] = firstMeta;
				while (currIdx < idx) {
					// if the node has parent push it
					ret.push(String(null));
					currIdx++;
				}
				ret.push(String(node.val));

				node.left && queue.push([idx * 2 + 1, node.left]);
				node.right && queue.push([idx * 2 + 2, node.right]);
				currIdx++;
			}
		}
		return ret;
	...
	return bfsResult
		.filter((val, idx) => {
			if (val === "null") {
				if (idx % 2 === 0 && bfsResult[(idx - 2) / 2] === "null") return false;
				if (idx % 2 === 1 && bfsResult[(idx - 1) / 2] === "null") return false;
			}
			return true;
		})
		.join(",");
...
function deserialize(data: string): TreeNode | null {
	...
	const treeArr: (number | null)[] = data.split(",").map((word) => {
		if (word === "null") return null;
		else return Number(word);
	});

	const rootVal = treeArr.shift();
	...
	while (queue.length !== 0) {
		...
		while (counts-- !== 0) {
			...
			const leftVal = treeArr.length === 0 ? null : treeArr.shift();
			const rightVal = treeArr.length === 0 ? null : treeArr.shift();
			...
			if (rightVal !== null) {
				first.right = new TreeNode(rightVal);
...
const treeNode = deserialize(c);
console.log(serialize(treeNode));
